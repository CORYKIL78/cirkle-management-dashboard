const REQUIRED_ROLE = '1315346851616002158'; // Manager role
const DEPT_ROLES = {
...
function showScreen(screenId) {
    ...
    if (screens[screenId]) {
        ...
        if (['mainMenu', 'absences', 'payslips', 'disciplinaries'].includes(screenId)) {
...
function addNotification(userId, type, message, link) {
    const empNotifications = JSON.parse(localStorage.getItem(`notifications_${userId}`)) || [];
    empNotifications.push({ id: Date.now().toString(), type, message, link, read: false, timestamp: new Date().toLocaleString() });
    localStorage.setItem(`notifications_${userId}`, JSON.stringify(empNotifications));
}
